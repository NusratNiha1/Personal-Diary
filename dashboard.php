...
<div class="container mx-auto px-4 py-8 max-w-7xl">
  ...
  <form method="get" class="space-y-4">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <!-- Search Box -->
      <div>
        <label class="block text-sm text-text-secondary mb-1">Search</label>
        <input type="text" name="search" value="<?php echo e($searchQuery); ?>" 
               placeholder="Search title or content..." 
               class="w-full rounded-2xl px-4 py-2 bg-white/10 text-white border border-white/15 focus:border-primary-500 outline-none shadow-sm transition placeholder-text-secondary" />
      </div>

      <!-- Mood Filter -->
      <div>
        <label class="block text-sm text-text-secondary mb-1">Mood</label>
        <select name="mood" class="w-full rounded-2xl px-4 py-2 bg-white/10 text-white border border-white/15 focus:border-primary-500 outline-none shadow-sm transition">
      ...
      <div>
        ...
        <select name="category" class="w-full rounded-2xl px-4 py-2 bg-white/10 text-white border border-white/15 focus:border-primary-500 outline-none shadow-sm transition">
      ...
      <div>
        ...
        <input type="date" name="date" value="<?php echo e($dateFilter); ?>" 
               class="w-full rounded-2xl px-4 py-2 bg-white/10 text-white border border-white/15 focus:border-primary-500 outline-none shadow-sm transition" />
      </div>
      
      <!-- Tag Filter -->
      <div>
        <label class="block text-sm text-text-secondary mb-1">Tag</label>
      ...
      <!-- Sort By -->
      <div>
        <label class="block text-sm text-text-secondary mb-1">Sort By</label>
        <select name="sort" class="w-full rounded-2xl px-4 py-2 bg-white/10 text-white border border-white/15 focus:border-primary-500 outline-none shadow-sm transition">
    ...
    <div class="flex justify-end gap-3 mt-4">
      <a href="dashboard.php" class="px-4 py-2 rounded-2xl bg-white/10 hover:bg-white/15 text-white border border-white/15 shadow-sm transition">
        Clear
      </a>
...
<div class="flex items-center justify-between mb-6">
  <h1 class="text-4xl font-bold text-white">Your Entries</h1>
...
<?php if (empty($entries)): ?>
  <div class="glass rounded-3xl p-8 text-center text-text-secondary">No entries yet. Create your first one!</div>
<?php else: ?>
  <div class="masonry">
      ...
      <a href="view.php?id=<?php echo (int)$e['entry_id']; ?>" class="block group masonry-item">
        <div class="glass rounded-3xl shadow-xl overflow-hidden hover:shadow-2xl transition relative">
          <?php if (!empty($e['mood'])): $emo = mood_emoji($e['mood']); ?>
            <div class="absolute top-3 right-3 z-10 px-2 py-1 rounded-full bg-white/20 text-white border border-white/30 text-xs flex items-center gap-1">
          ...
          <div class="p-5">
            <div class="flex items-center justify-between">
              <h2 class="text-lg font-semibold text-white truncate"><?php echo e($e['title']); ?></h2>
            </div>
            ...
            <p class="text-sm text-text-secondary mt-2 line-clamp-2"><?php echo e(mb_strimwidth($e['content'], 0, 120, '…')); ?></p>
            
            <?php if (!empty($e['tags'])): ?>
              <div class="mt-2 flex flex-wrap gap-1">
                <?php $tag_list = explode(', ', $e['tags']); foreach (array_slice($tag_list, 0, 3) as $tag): ?>
                  <span class="inline-block px-2 py-0.5 rounded-full bg-white/10 text-text-secondary text-xs">
                    #<?php echo e($tag); ?>
                  </span>
                <?php endforeach; ?>
                <?php if (count($tag_list) > 3): ?>
                  <span class="text-xs text-text-secondary">+<?php echo count($tag_list) - 3; ?></span>
                <?php endif; ?>
              </div>
            <?php endif; ?>
            
            <p class="text-xs text-text-secondary mt-3"><?php echo e(human_datetime($e['timestamp'])); ?></p>
          </div>
        </div>
      </a>
    <?php endforeach; ?>
