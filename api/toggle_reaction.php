...
try {
  ...
  if ($existing) {
    ...
    // Update entry stats manually (replaces trigger)
    $pdo->prepare('UPDATE entry_stats SET reaction_count = GREATEST(reaction_count - 1, 0) WHERE entry_id = ?')->execute([$entryId]);
    ...
    update_entry_stat_count($entryId, 'reaction_count');
